...
const VERIFY_DESCRIPTION =
  'Pause here if someone contacted you about a CAP grant. Verifying means confirming the text or email is from CAP before you reply, share personal information, or take any next step.';

const verifyFaqs = [
  {
    question: 'How do I verify a CAP grant text or email?',
  ...
  },
  {
    ...
    answer:
      'No. CAP grants are not loans. We will never ask for your email or banking passwords. If anyone claiming to represent us demands passwords or repayment as if the award were a loan, ignore them and write support.',
  },
  {
...
export const metadata = createMetadata({
  title: 'Verify a CAP Grant Text or Email',
...
export default function VerifyPage() {
  return (
    <div className="bg-white">
      <JsonLd data={webPageJsonLd({ name: 'Verify official CAP contact', path: '/verify', description: VERIFY_DESCRIPTION })} />
      <JsonLd
      ...
      <section className="border-b border-[var(--cap-line)] bg-[var(--cap-paper)] pt-16 pb-12">
        <div className="container-page max-w-3xl text-center">
          ...
          <h1 className="mt-3 text-4xl">
            Someone contacted you about a CAP grant?
            <span className="mt-2 block text-[var(--cap-accent)]">Verify it here first.</span>
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-[var(--cap-muted)]">
            If a text or email about a CAP grant left you unsure, pause and write us. Verifying
      ...
      <section className="py-12">
        <div className="container-page max-w-3xl space-y-8">
          <div className="border border-[var(--cap-line)] p-8 text-center">
            <CheckCircle2 className="mx-auto h-10 w-10 text-[var(--cap-accent)]" />
            <h2 className="mt-4 text-xl">Official support email</h2>
            <a
              href={`mailto:${CONTACT_EMAIL}?subject=Please%20verify%20this%20contact`}
              className="mt-2 block text-lg font-semibold text-[var(--cap-accent)]"
            >
              {CONTACT_EMAIL}
            </a>
          ...
          <div>
            ...
            <div className="mt-4">
              {verifyFaqs.map((item) => (
                <div key={item.question} className="border-t border-[var(--cap-line)] py-5">
                  <h3 className="text-sm font-semibold text-[var(--cap-blue)]">{item.question}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-[var(--cap-muted)]">{item.answer}</p>
                </div>
              ))}
            </div>
          </div>
