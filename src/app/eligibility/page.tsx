...
export const metadata = createMetadata({
  title: 'Who Can Apply for a CAP Grant',
  description:
    'Adults 18 or older may apply. No income cap or means test. CAP grants support medical care, housing, business, school, teachers, and seniors. Grants are not loans.',
  path: '/eligibility',
});
  ...
  return (
    <div className="bg-white">
      ...
      <section className="py-16">
        <div className="container-page max-w-3xl space-y-8 text-sm leading-relaxed text-[var(--cap-muted)]">
          <p>
          ...
          <ul className="space-y-4">
            {GRANT_PROGRAMS.map((p) => (
              <li key={p.slug} className="border-t border-[var(--cap-line)] pt-4">
                <Link href={`/programs/${p.slug}`} className="font-semibold text-[var(--cap-blue)]">
                  {p.title}
                </Link>
          ...
          <h2 className="text-2xl text-[var(--cap-accent)]">What does not make a file a loan</h2>
          <p>
            CAP Grants are not loans. Selection does not create monthly payments to CAP.
