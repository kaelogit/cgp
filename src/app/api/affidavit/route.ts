...
export async function POST(request: NextRequest) {
  ...
  const operatorHtml = `
    <h2>CAP Affidavit of Eligibility — Submitted Online</h2>
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `[CAP Affidavit] ${payload.fullName} — ${payload.supportAmount}`,
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>This confirmation is not a final funding release. CAP awards are grants, not loans.</p>
        <p>${escapeHtml(COORDINATOR_NAME)}<br/>${escapeHtml(COORDINATOR_TITLE)}<br/>${escapeHtml(FULL_NAME)}<br/>${escapeHtml(COORDINATOR_EMAIL)}</p>
      `,
      text: `Dear ${payload.fullName},\n\nWe have received your Affidavit of Eligibility and Release. ${COORDINATOR_NAME} will email you with the next step from ${COORDINATOR_EMAIL}.\n\n${FULL_NAME}\n${COORDINATOR_EMAIL}`,
    });

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('CAP affidavit submit error:', err);
