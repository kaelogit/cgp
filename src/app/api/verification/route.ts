...
export async function POST(request: NextRequest) {
  ...
  const html = `
    <h2>CAP Applicant Verification Form — Submitted</h2>
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `[CAP Verification] ${payload.fullName} — ${payload.category}`,
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>This confirmation only means we received your form — it is not a funding decision. CAP awards are grants, not loans.</p>
        <p>${escapeHtml(COORDINATOR_NAME)}<br/>${escapeHtml(COORDINATOR_TITLE)}<br/>${escapeHtml(FULL_NAME)}<br/>${escapeHtml(COORDINATOR_EMAIL)}</p>
      `,
      text: `Dear ${payload.fullName},\n\nWe have received your Applicant Verification Form. ${COORDINATOR_NAME} will email you with the next step from ${COORDINATOR_EMAIL}.\n\n${FULL_NAME}\n${COORDINATOR_EMAIL}`,
    });

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('CAP verification submit error:', err);
