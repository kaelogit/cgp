...
export async function generateMetadata({ params }: Props) {
  ...
  return createMetadata({
    title: `${program.title} — CAP Grant, Not a Loan`,
    description: `${program.summary} This is a CAP grant, not a loan.`,
    ...
    <div className="bg-white">
      ...
      <section className="py-16">
        <div className="container-page max-w-3xl space-y-6">
          <p className="text-sm leading-relaxed text-[var(--cap-muted)]">{program.details}</p>
          <p className="text-sm leading-relaxed text-[var(--cap-muted)]">
            CAP Grants are not loans. If you are selected, you do not repay the award.
            Applying does not guarantee funds. Write plainly. Completeness matters more than perfect
            wording.
          </p>
          {related.length > 0 ? (
            <div className="border-t border-[var(--cap-line)] pt-8">
              <h2 className="text-xl">Letters in this category</h2>
              <ul className="mt-4 space-y-4">
                {related.map((s) => (
                  <li key={s.name}>
                    <p className="font-semibold text-[var(--cap-blue)]">
                      {s.name} · {s.amount}
                    </p>
                    <p className="text-sm text-[var(--cap-muted)]">“{s.quote}”</p>
                  </li>
                ))}
              </ul>
              <Link href="/impact" className="mt-4 inline-block text-sm font-semibold text-[var(--cap-accent)]">
                All stories
              </Link>
            </div>
          ) : null}
